...
あたしは半年前にこの問題にぶち当たって、「ラズパイで全部オンプレにすればいいのでは？」と思い立ちました。消費電力5〜15W、月の電気代100円以下、24時間つけっぱなしで壊れない。最初は半信半疑でしたが、結果的に**月500円以下**で自律AIエージェントが動く環境が出来上がりました。
...
ポイントは**無料枠の活用**です。Geminiは複数のGoogleプロジェクトでそれぞれAPIキーを発行でき、Groqも別プロジェクトでキーを分けることでクォータを分散しています。これで課金が必要なのはGeminiの1キーだけで、月数百円で済んでいます。

:::message
各サービスの利用規約でクォータ回避目的の複数アカウント作成が禁止されている場合があります。あたしの場合はGoogleの別プロジェクト（同一アカウント）やGroqの別Project（同一アカウント）での運用ですが、規約は各自ご確認ください。
:::
...
**信頼度スコア**という仕組みがあって、エージェントが「この判断は自分でやっていいか」を数値で評価します。閾値を超えれば自動実行、超えなければ人間に承認を求める。あたしが寝てる間にやっていいことと、起きてるときに確認が必要なことをエージェント自身が判断してくれます。
